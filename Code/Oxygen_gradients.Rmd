...
linear_fits <- Gradients %>%
  group_by(Sample) %>%
  group_map(~ {
    model <- lm(Voltage_Water_10s ~ Depth_cm, data = .x)  # Fit the linear model
    tidy_model <- tidy(model)  # Get the tidy model summary
    tidy_model$Sample <- unique(.x$Sample)  # Add the 'Sample' variable
    return(tidy_model)  # Return the tidy model with 'Sample'
  }, .keep = TRUE) %>%
  bind_rows() %>%
  filter(term == "Depth_cm")  # Filter for 'Depth_cm' coefficient
